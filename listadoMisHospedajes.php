...
<div class="container">
    <h1 class="text-center"> Mis hospedajes </h1>
    <?php
    include("conectarBD.php");
    $queryHospedajes = "SELECT nombre_tipo, couch.id_usuario, titulo, descripcion, couch.id_couch, ubicacion, direccion,  DATE_FORMAT(finicio, '%d-%m-%y') AS finicio, DATE_FORMAT(ffin,'%d-%m-%y') AS ffin FROM reserva INNER JOIN couch ON (reserva.id_couch = couch.id_couch) INNER JOIN tipo ON (couch.id_tipo = tipo.id_tipo) WHERE estado='finalizada' AND reserva.id_usuario='" . $_SESSION['id_usuario'] . "' ";
