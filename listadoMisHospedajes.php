...
<div class="container">
    ...
    $queryHospedajes = "SELECT couch.despublicado, eliminado_couch, nombre_tipo, couch.id_usuario, titulo, descripcion,capacidad, couch.id_couch, ubicacion, direccion,  DATE_FORMAT(finicio, '%d-%m-%y') AS finicio, DATE_FORMAT(ffin,'%d-%m-%y') AS ffin FROM reserva INNER JOIN couch ON (reserva.id_couch = couch.id_couch) INNER JOIN tipo ON (couch.id_tipo = tipo.id_tipo) WHERE estado='finalizada' AND reserva.id_usuario='" . $_SESSION['id_usuario'] . "' ";
                    ...
                    <div class="row" style="padding-left:10px">
                        ...
                        <div class="col-md-6" style="padding-left: 50px"><h4 class="list-group-item-heading"><?php echo($hospedaje['titulo']); ?></h4>
                            <p class="list-group-item-text">
                                <strong>Descripción: </strong><?php echo($hospedaje['descripcion']);?><br>
                                 <strong>Ubicación: </strong><?php echo($hospedaje['ubicacion']);?><br>
                                  <strong>Dirección: </strong><?php echo($hospedaje['direccion']) ?><br>
                                  <strong>Capacidad: </strong> para <?php echo($hospedaje['capacidad']); ?> personas <br>
                                  <strong>Tipo: </strong><?php echo($hospedaje['nombre_tipo']);?><br>
                                  <strong> Inicio estadia: </strong><?php echo($hospedaje['finicio']);?><br>
                                  <strong>Fin estadia: </strong><?php echo($hospedaje['ffin']);?>
                            </p>
                        </div>
